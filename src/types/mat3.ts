...
export const mat3 = createImpl(
  class Mat3Impl extends Static {
    ...
    static copy<T extends num3x3>(a: T, b: num3x3) {
      return mat3.set(a, b.c0.x, b.c0.y, b.c0.z, b.c1.x, b.c1.y, b.c1.z, b.c2.x, b.c2.y, b.c2.z);
    }
    /** Clone `m` matrix */
    ...
    static fromCols(x: num3, y: num3, z: num3, out = mat3()) {
      return mat3.set(out, x.x, x.y, x.z, y.x, y.y, y.z, z.x, z.y, z.z);
    }
    ...
    static fromArray(array: ArrayLike<number>, out = mat3()) {
      return mat3.set(out, array[0], array[1], array[2], array[3], array[4], array[5], array[6], array[7], array[8]);
    }
    ...
    static fromArray2d(array: [ArrayLike<number>, ArrayLike<number>, ArrayLike<number>], out = mat3()) {
      return mat3.fromCols(vec3.fromArray(array[0], v1), vec3.fromArray(array[1], v2), vec3.fromArray(array[2], v3), out);
    }
    ...
    static fromDiagonal(diagonal: num3, out = mat3()) {
      return mat3.set(out, diagonal.x, 0, 0, 0, diagonal.y, 0, 0, 0, diagonal.z);
    }
    ...
    static fromRotationX(radians: Rad, out = mat3()) {
      let [sin, cos] = sincos(radians);
      return mat3.fromCols(vec3.right(), vec3.set(v1, 0, cos, sin), vec3.set(v2, 0, -sin, cos), out);
    }
    ...
    static fromRotationY(radians: Rad, out = mat3()) {
      let [sin, cos] = sincos(radians);
      return mat3.fromCols(vec3.set(v1, cos, 0, -sin), vec3.up(), vec3.set(v2, sin, 0, cos), out);
    }
    ...
    static fromRotationZ(radians: Rad, out = mat3()) {
      let [sin, cos] = sincos(radians);
      return mat3.fromCols(vec3.set(v1, cos, sin, 0), vec3.set(v2, -sin, cos, 0), vec3.forward(), out);
    }
    ...
    static fromTranslation(translation: num2, out = mat3()) {
      return mat3.fromCols(vec3.right(), vec3.up(), vec3.set(v1, translation.x, translation.y, 1.0), out);
    }
    ...
    static fromScale(scale: num2, out = mat3()) {
      ...
      return mat3.fromCols(vec3.set(v1, scale.x, 0, 0), vec3.set(v2, 0, scale.y, 0), vec3.forward(), out);
    }
    ...
    static toColsArray(m: num3x3): ColsArray {
      return [m.c0.x, m.c0.y, m.c0.z, m.c1.x, m.c1.y, m.c1.z, m.c2.x, m.c2.y, m.c2.z];
    }
    ...
    static add(lhs: num3x3, rhs: num3x3, out = mat3()) {
      return mat3.fromCols(vec3.add(lhs.c0, rhs.c0, v1), vec3.add(lhs.c1, rhs.c1, v2), vec3.add(lhs.c2, rhs.c2, v3), out);
    }
    ...
    static sub(lhs: num3x3, rhs: num3x3, out = mat3()) {
      return mat3.fromCols(vec3.sub(lhs.c0, rhs.c0, v1), vec3.sub(lhs.c1, rhs.c1, v2), vec3.sub(lhs.c2, rhs.c2, v3), out);
    }
    ...
    static mul(lhs: num3x3, rhs: num3x3, out = mat3()) {
      return mat3.fromCols(mat3.vmul3(lhs, rhs.c0, v1), mat3.vmul3(lhs, rhs.c1, v2), mat3.vmul3(lhs, rhs.c2, v3), out);
    }
    ...
    static scale(lhs: num3x3, rhs: number, out = mat3()) {
      return mat3.fromCols(vec3.scale(lhs.c0, rhs, v1), vec3.scale(lhs.c1, rhs, v2), vec3.scale(lhs.c2, rhs, v3), out);
    }
    ...
    static row<T extends num3x3>(m: T, index: number): vec3 {
      switch (index) {
        ...
        case 1:
          return vec3(m.c0.y, m.c1.y, m.c2.y);
        case 2:
          return vec3(m.c0.z, m.c1.z, m.c2.z);
        default:
          panic(`index out of range: ${index}`);
      }
    }
    ...
    static inv(m: num3x3, out = mat3()) {
      ...
      return mat3.transpose(mat3.fromCols(vec3.mul(cr0, invDet, cr0), vec3.mul(cr1, invDet, cr1), vec3.mul(cr2, invDet, cr2), out));
    }
    ...
    static transformVec2(m: num3x3, rhs: num2, out = vec2()) {
      return mat2.vmul2(mat2.fromCols(vec3.xy(m.c0, v1), vec3.xy(m.c1, v2), m1), rhs, out);
    }
  },
);
...
/** @internal */
type ColsArray = [m00: number, m01: number, m02: number, m10: number, m11: number, m12: number, m20: number, m21: number, m22: number];
