...
/** Operations for a 2-dimensional vector. */
export const vec2 = createType({
    ...
    dot(lhs: struct.Vec2, rhs: struct.Vec2): number {
        return lhs.x * rhs.x + lhs.y * rhs.y;
    },
    /** Returns the cross product of given two vectors. */
    cross(lhs: struct.Vec2, rhs: struct.Vec2): number {
        return lhs.x * rhs.y - lhs.y * rhs.x;
    },
    /** Returns the vector projection of `lhs` onto `rhs`. */
    project(lhs: struct.Vec2, rhs: struct.Vec2, out = create()): struct.Vec2 {
    ...
    reflect(lhs: struct.Vec2, rhs: struct.Vec2, out = create()): struct.Vec2 {
        assert(this.isNormalized(rhs), "must be normalized");
        return this.sub(lhs, this.project(lhs, rhs, v1), out);
    },
    /** Returns the angle (in radians) between the two vectors. */
    angle(lhs: struct.Vec2, rhs: struct.Vec2): number {
    ...
    rotate(target: struct.Vec2, radians: number, out = create()): struct.Vec2 {
    ...
    lerp(a: struct.Vec2, b: struct.Vec2, t: number, out = create()): struct.Vec2 {
        return this.add(a, this.scale(this.sub(b, a, v1), t, v1), out);
    }
});

/** A 2-dimensional vector. */
export class Vec2 implements struct.Vec2 {
    x: number;
    y: number;
    /** Creates a 2-dimensional vector. */
    ...
    /** Copies properies from `other` */
    copy(other: struct.Vec2): Vec2 {
        return vec2.copy(this, other) as Vec2;
    }
    ...
    extend(z: number = 0, out = { x: 0, y: 0, z: 0 }): vec3 {
        return vec2.extend(this, z, out) as vec3;
    }
