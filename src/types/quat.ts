...
export const quat = createImpl(
  class QuatImpl extends Static {
    ...
    static create(x: number, y: number, z: number, w: number) {
      let v = x ?? 0;
      return new Quat(x ?? v, y ?? v, z ?? v, w ?? x ?? 1);
    }
    ...
    static fromEuler(rot: num3, euler: EulerRot = EulerRot.ZXY, out = quat()) {
      ...
      switch (euler) {
        ...
        default:
          panic(`Unknown euler rotation: ${euler}`);
      }
    }
    ...
    static fromAxes(x: num3, y: num3, z: num3, out = quat()) {
      ...
      if (m22 <= 0.0) {
        ...
        if (dif10 <= 0) {
          ...
          return quat.set(out, fourXsq * inv4x, (m01 + m10) * inv4x, (m02 + m20) * inv4x, (m12 - m21) * inv4x);
        } else {
          let fourYsq = omm22 + dif10;
          let inv4y = 0.5 / sqrt(fourYsq);
          return quat.set(out, (m01 + m10) * inv4y, fourYsq * inv4y, (m12 + m21) * inv4y, (m20 - m02) * inv4y);
        }
      } else {
        // z^2 + w^2 >= x^2 + y^2
        let sum10 = m11 + m00;
        ...
        if (sum10 <= 0.0) {
          ...
          return quat.set(out, (m02 + m20) * inv4z, (m12 + m21) * inv4z, fourZsq * inv4z, (m01 - m10) * inv4z);
        } else {
          let fourWsq = opm22 + sum10;
          let inv4w = 0.5 / sqrt(fourWsq);
          return quat.set(out, (m12 - m21) * inv4w, (m20 - m02) * inv4w, (m01 - m10) * inv4w, fourWsq * inv4w);
        }
      }
    }
