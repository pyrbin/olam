...
export const mat2 = createImpl(
  class Mat2Impl extends Static {
    ...
    static add(lhs: num2x2, rhs: num2x2, out = mat2()) {
      return mat2.fromCols(vec2.add(lhs.c0, rhs.c0, v1), vec2.add(lhs.c1, rhs.c1, v2), out);
    }
    ...
    static sub(lhs: num2x2, rhs: num2x2, out = mat2()) {
      return mat2.fromCols(vec2.sub(lhs.c0, rhs.c0, v1), vec2.sub(lhs.c1, rhs.c1, v2), out);
    }
    ...
    static mul(lhs: num2x2, rhs: num2x2, out = mat2()) {
      return mat2.fromCols(mat2.vmul2(lhs, rhs.c0, v1), mat2.vmul2(lhs, rhs.c1, v2), out);
    }
    ...
    static vmul2(lhs: num2x2, rhs: num2, out = vec2()) {
      return vec2.set(out, lhs.c0.x * rhs.x + lhs.c1.x * rhs.y, lhs.c0.y * rhs.x + lhs.c1.y * rhs.y);
    }
    ...
    static scale(lhs: num2x2, rhs: number, out = mat2()) {
      return mat2.fromCols(vec2.scale(lhs.c0, rhs, v1), vec2.scale(lhs.c1, rhs, v2), out);
    }
    ...
    static row<T extends num2x2>(m: T, index: number): vec2 {
      switch (index) {
        case 0:
          return vec2(m.c0.x, m.c1.x);
        case 1:
          return vec2(m.c0.y, m.c1.y);
        default:
          panic(`index out of range: ${index}`);
      }
    }
    ...
    static inv(m: num2x2, out = mat2()) {
      ...
      let inv = recip(det);
      return mat2.fromCols(vec2.set(v1, inv * m.c1.y, -inv * m.c0.y), vec2.set(v2, -inv * m.c1.x, inv * m.c0.x), out);
    }
    ...
    static transpose(m: num2x2, out = mat2()) {
      return mat2.fromCols(vec2.set(v1, m.c0.x, m.c1.x), vec2.set(v2, m.c0.y, m.c1.y), out);
    }
...
/** @internal */
type ColsArray2d = [c0: [ColsArray[0], ColsArray[1]], c1: [ColsArray[2], ColsArray[3]]];
